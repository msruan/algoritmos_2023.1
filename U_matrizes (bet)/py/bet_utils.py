...
def obter_resposta_s_ou_n(label='Sim ou Não?'):
...
def obter_opcao(tamanho_minimo, tamanho_maximo,menu):
    limpar_tela()
    num = input(menu)
    
    if num =='':
        return obter_inteiro_faixa(tamanho_minimo, tamanho_maximo, menu)
    num = inteiro(num)
    if (tamanho_minimo <= num and num <= tamanho_maximo):
        return num
    return obter_inteiro_faixa(tamanho_minimo, tamanho_maximo, menu)
...
def obter_inteiro_faixa_com_excecao(tamanho_minimo, tamanho_maximo,excecao,label='Insira um número: '):
    if label == '':#mensagem vazia?
        num = obter_inteiro(f'Por favor, insira um número de ho mínimo {tamanho_minimo} e de máximo {tamanho_maximo}: ')
    else:
        num = obter_inteiro(label)
    if tamanho_minimo > num or num > tamanho_maximo or num == excecao:
        return obter_inteiro_faixa_com_excecao(tamanho_minimo, tamanho_maximo, excecao, label)
    return num
...
def revert(lista_recebida):
    ...
    for i in range(len(lista_recebida)):
...
def mostrar_bilhete(bilhete):
    vetor_visual =f'{colored("|","black")}'
    for i in range(len(bilhete)):
        vetor_visual += ((f' {bilhete[i]}'+' '+f'{ colored("|","black")}') if (bilhete[i] > 9) else (f' 0{bilhete[i]}') +' '+f'{ colored("|","black")}')
    return vetor_visual

def mostrar_sorteados_no_bilhete(bilhete,sorteados):
    vetor_visual =f'{colored("|","black")}'
    for i in range(6):
        if bilhete[i] > 9:
            vetor_visual += ((f' {bilhete[i]}' f' {colored("|","black")}' if (bilhete[i] not in sorteados) else (f' {colored_cyan(str(bilhete[i]))}') +f' {colored("|","black")}'))
        else:
            vetor_visual += (((f' 0{bilhete[i]}'+ f' {colored("|","black")}') if (bilhete[i] not in sorteados) else (f' 0{colored_cyan(str(bilhete[i]))}') + f' {colored("|","black")}'))
    return vetor_visual
...
#happy utils
def interjeicao():
    interjeicoes = ["Samba coração!","Oh...","Curioso...","Haja coração!","Aguenta coração!","Toma essa!","Uau!","Segura essa!","Oh não!","Nem a Raven previu!","Incrível!","Perdi tudo!","Chocado!","Catapimbas","Quem diria!","Nossa!", "Caramba!", "Meu Deus!", "Passado!", "Impressionante!", "Que incrível!", "Uau, não acredito!", "Céus!", "Nossa senhora!", "Puxa!", "Não posso acreditar!", "Santo Deus!", "Carambolas!", "Cara!", "Puxa vida!", "Nossa, isso é demais!", "Uau, que loucura!", "Jesus!"]
    return (interjeicoes[gerar_randomico(0,32)])

def bye():
    lista = ["Adeus, minha pimenta!", "Até mais, meu biscoitinho!", "Tchau, meu docinho de coco!", "Adeus, meu abacate!", "Até a próxima, meu brigadeiro!", "Tchau, meu pão de queijo!", "Adeus, meu bombonzinho!", "Até logo, minha pipoca!", "Tchauzinho, meu sorvetinho!", "Adeus, minha paçoquinha!", "Até mais, minha marmita!", "Tchau, meu cupcake!", "Adeus, meu marshmallow!", "Até a próxima, minha gelatina!", "Tchau, meu bolo de cenoura!", "Adeus, minha cocada!", "Até logo, minha torta de limão!", "Tchauzinho, meu suspiro!", "Adeus, meu chiclete!", "Até mais, meu pastel com caldo de cana!", "Tchau, minha coxinha de frango!", "Adeus, meu milkshake!", "Até a próxima, meu churros!", "Tchau, minha paçoca!", "Adeus, meu brigadeiro de panela!", "Até logo, minha geléia!", "Tchauzinho, meu crepe!", "Adeus, minha salada de frutas!", "Até mais, minha limonada!", "Tchau, meu biscoito da sorte!", "Adeus, meu sushi!", "Até a próxima, meu pato no tucupi!", "Tchau, meu rolinho primavera!", "Adeus, meu macarrão!", "Até logo, meu bacon!", "Tchauzinho, meu frango a passarinho!", "Adeus, meu hambúrguer!", "Até mais, meu cachorro-quente!", "Tchau, minha pizza!", "Adeus, meu sorvete de casquinha!", "Até a próxima, meu milkshake!", "Tchau, meu bolo de chocolate!", "Adeus, meu croissant!", "Até logo, meu pão francês!", "Tchauzinho, meu waffle!", "Adeus, minha tapioca!", "Até mais, meu pão de mel!", "Tchau, meu brownie!", "Adeus, meu mousse!", "Até a próxima, minha geleia de mocotó!", "Tchau, meu macaron!", "Adeus, meu cuscuz!", "Até logo, meu panetone!", "Tchauzinho, meu brigadeiro branco!", "Adeus, minha coxinha!", "Até mais, meu pastel de feira!", "Tchau, minha empadinha!", "Adeus, meu esfiha!", "Até a próxima, meu churros!", "Tchau, meu pão de queijo!", "Adeus, minha tapioca!", "Até logo, minha geléia!", "Tchauzinho, meu brownie!", "Adeus, meu mousse!", "Até mais"]
