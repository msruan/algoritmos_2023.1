...
def pesquisa(quantidade):
    ...
    show('')     
    show('=' * 50,'\n') 
    show(f'\nA média de salário é {soma_salarios / quantidade}')
    show(f'A média de número de filhos é {contador_filhos / quantidade}')
    show(f'O percentual de pessoas que ganham até 1000 reais é de {percentual(contador_salario_min, quantidade)}%\n')
    show('=' * 50,'\n') 
